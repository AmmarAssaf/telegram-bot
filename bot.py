...
import sqlite3  # ✅ استخدام SQLite بدلاً من PostgreSQL
import random
import string
import os
import json
...
# ==============================
# 🗄️ إعدادات قاعدة البيانات SQLite
# ==============================
...
def setup_database():
    """إنشاء الجداول المطلوبة في قاعدة البيانات SQLite"""
    try:
        ...
        conn.commit()
        conn.close()
        logger.info("✅ تم إعداد قاعدة البيانات والجداول بنجاح على SQLite!")
        return True
...
def save_registration_progress(user_id: int, current_stage: str, user_data: dict):
    ...
    try:
        ...
        cursor.execute('''
            INSERT OR REPLACE INTO registration_progress 
...
# ... (استمر بنفس الكود الأصلي لكل دوال المحادثة مع تعديل الاستعلامات ل SQLite)
...
def main():
    ...
    print("🤖 البوت الكامل يعمل الآن على Render!")
    print("💡 نظام التسجيل المتكامل جاهز للاستخدام")
