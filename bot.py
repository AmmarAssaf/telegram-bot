...
import psycopg2  # ✅ تغيير من pyodbc إلى psycopg2
...
# ==============================
# 🗄️ إعدادات قاعدة البيانات (نسخة Render المعدلة)
# ==============================
...
# إعدادات قاعدة البيانات المرنة للسحابة
def get_database_config():
    ...
    try:
        ...
        if 'DATABASE_URL' in os.environ:
            ...
            # تحويل من postgres:// إلى postgresql://
            ...
            return {
                ...
                'environment': 'render',
                'connection_string': database_url  # ✅ إضافة connection string كامل
            }
        else:
            # البيئة المحلية (للاختبار فقط)
            return {
                'driver': 'SQL Server',
...
def create_connection():
    """إنشاء اتصال بقاعدة البيانات - متوافق مع كلا البيئتين"""
    try:
        ...
        if config['environment'] == 'render':
            # ✅ اتصال PostgreSQL مع Render
            return psycopg2.connect(config['connection_string'])
        else:
            # ❌ البيئة المحلية (لن تعمل على Render لكن نتركها للتوافق)
            import pyodbc
...
# ==============================
(
    REFERRAL_STAGE, FULL_NAME, COUNTRY, GENDER, BIRTH_YEAR, PHONE, EMAIL,
    SOCIAL_MEDIA_MENU, FACEBOOK_URL, INSTAGRAM_URL, YOUTUBE_URL, OTHER_SOCIAL_MEDIA,
    PAYMENT_METHOD, WALLET_TYPE, WALLET_ADDRESS, NEW_WALLET_TYPE, TRANSFER_DETAILS,
    TRANSFER_PHONE, TRANSFER_LOCATION, TRANSFER_COMPANY, CONFIRMATION, EDIT_CHOICE,
    EDIT_FULL_NAME, EDIT_COUNTRY, EDIT_GENDER, EDIT_BIRTH_YEAR, EDIT_PHONE,
    EDIT_EMAIL, EDIT_SOCIAL_MEDIA, EDIT_PAYMENT_METHOD
) = range(30)
...
# ==============================
# 🗃️ دوال قاعدة البيانات (معدلة ل PostgreSQL)
# ==============================

def setup_database():
    """إنشاء الجداول المطلوبة في قاعدة البيانات PostgreSQL"""
    try:
        ...
        conn.commit()
        conn.close()
        logger.info("✅ تم إعداد قاعدة البيانات والجداول بنجاح على PostgreSQL!")
        return True
...
def save_registration_progress(user_id: int, current_stage: str, user_data: dict):
    ...
    try:
        ...
        # استخدام UPSERT في PostgreSQL
        cursor.execute('''
            INSERT INTO registration_progress (user_id, current_stage, user_data, telegram_username)
            VALUES (%s, %s, %s, %s)
            ON CONFLICT (user_id) 
            DO UPDATE SET 
        ...
        ''', (user_id, current_stage, json.dumps(user_data), user_data.get('telegram_username', '')))
...
# ==============================
# 🔍 دوال التحقق من الصحة (نفسها بدون تغيير)
# ==============================
...
async def start(update: Update, context: CallbackContext) -> int:
    """بدء عملية التسجيل - معدل للعمل مع PostgreSQL"""
    ...
    if not create_connection():
        await update.message.reply_text(
            ...
            "⚠️ يرجى المحاولة لاحقاً أو التواصل مع الدعم الفني."
        )
...
async def save_all_data(update: Update, context: CallbackContext):
    """حفظ جميع البيانات في قاعدة البيانات PostgreSQL"""
    try:
        conn = create_connection()
        if not conn:
        ...
        logger.info(f"✅ تم حفظ بيانات المستخدم {user_id} بنجاح على PostgreSQL")
        context.user_data['referral_code'] = referral_code
        
    except Exception as e:
        logger.error(f"❌ خطأ في حفظ البيانات على PostgreSQL: {e}")
        await update.message.reply_text("❌ حدث خطأ في حفظ البيانات")
...
async def show_profile(update: Update, context: CallbackContext):
    """عرض الملف الشخصي للمستخدم - معدل ل PostgreSQL"""
    try:
        user_id = update.effective_user.id
...
def main():
    ...
    print("🚀 بدء إعداد البوت على Render...")
    ...
    print(f"🌍 البيئة: {DB_CONFIG['environment']}")
    ...
    # إعداد نظام المحادثات الرئيسي (نفس الكود)
    ...
    print("🤖 البوت المعدل للعمل على Render جاهز للتشغيل!")
    print("💡 سيتم تشغيل البوت على السحابة بدون أي مشاكل في قاعدة البيانات")
